...
/********************************************/
/* Archivo que contiene el main del progama */
/*   donde se realiza todo lo referente al  */
/* 		proceso maestro y los hilos			*/
/********************************************/
	...
	while ((pDirent = readdir(pDir)) != NULL) {
		...
		if ((tipo == 4) && (flag == 1)) {
			if (strcmp (pDirent->d_name, "..") != 0 && strcmp (pDirent -> d_name, ".") != 0) {
				if(strcmp(dir,".") == 0){
					snprintf (ruta, 500,"%s/%s/", dir, pDirent->d_name);
				}else{
					snprintf (ruta, 500,"%s%s/", dir, pDirent->d_name);
				}
